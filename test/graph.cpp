...
static UPTKGraphNode_t add_empty_node(UPTKGraph_t graph, UPTKError_t* out_ret) {
    ...
    // runtime 已实现 UPTKGraphAddEmptyNode（UPTKGraphAddNode 未实现）
    UPTKError_t ret = UPTKGraphAddEmptyNode(&node, graph, NULL, 0);
...
static void test_GraphAddNode_GetNodes_NodeGetType_DestroyNode() {
    ...
    // Some backends don't support empty nodes via UPTKGraphAddEmptyNode.
...
static void test_GraphInstantiate_Launch_ExecDestroy() {
    ...
    UPTKError_t ret_inst = UPTKGraphInstantiate(&exec, graph, 0);
