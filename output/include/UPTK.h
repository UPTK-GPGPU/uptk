/*
 * NOTE: We deliberately share the same header guard with UPTK_runtime_api.h
 * (__UPTK_RUNTIME_API_H__). UPTK.h is a "fat" public header that already
 * defines the same enums/structs as the UPTK_runtime_api.h + UPTK_driver_types.h
 * chain. Sharing the guard makes the two mutually exclusive in any single
 * translation unit and prevents the "multiple definition / redefinition"
 * compile errors that used to occur when both pulls were active at once.
 */
#ifndef __UPTK_RUNTIME_API_H__
...
struct  UPTKGraphNodeParams {
 ...
 #define UPTKKernelNodeAttrValue UPTKLaunchAttributeValue
    ...
    extern __host__ UPTKError_t UPTKIpcOpenMemHandle(CUdeviceptr * devPtr, UPTKIpcMemHandle_t handle, unsigned int flags);
...
//UPTKError  UPTKStreamSynchronizeD(UPTKStream_t * hStream);
//UPTKError  UPTKEventElapsedTime(float *ms, UPTKEvent_t start, UPTKEvent_t end);
//UPTKError  UPTKEventElapsedTimeD(float * pMilliseconds, UPTKEvent_t hStart, UPTKEvent_t hEnd);
...
UPTKError  UPDeviceGet(CUdevice * device,int ordinal);
